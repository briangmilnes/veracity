...
//!
//! Usage:
//!   veracity-minimize-proofs -c /path/to/codebase -l /path/to/library
//!   veracity-minimize-proofs -c /path/to/codebase -l /path/to/library --dry-run
//!
//! Binary: veracity-minimize-proofs
//!
//! Logs to: analyses/veracity-minimize-proofs.YYYYMMDD-HHMMSS.log
...
fn init_logging(codebase: &Path, chapter: Option<&str>) -> (PathBuf, bool) {
...
impl MinimizeArgs {
    ...
    fn print_usage(program_name: &str) {
        ...
        log!();
        log!("Minimize proof dependencies for Verus verification");
        log!();
        log!("Options:");
    ...
    for line in &lines {
        ...
        if trimmed.starts_with("// Veracity:") {
            ...
            if marker_trimmed == "USED" ||
               ...
               marker_trimmed == "TYPE USED" ||
               marker_trimmed == "NEEDED assert" ||
               marker_trimmed == "NEEDED admit" ||
               marker_trimmed == "NEEDED proof block" {
                markers_removed += 1;
            ...
            let code_start = if marker_content.starts_with("UNUSED ") {
                ...
                Some("TESTING ".len())
            } else if marker_content.starts_with("TESTING admit ") {
                Some("TESTING admit ".len())
            } else if marker_content.starts_with("TESTING proof block ") {
                Some("TESTING proof block ".len())
            } else if marker_content.starts_with("TESTING type ") {
                Some("TESTING type ".len())
            } else if marker_content.starts_with("TYPE UNUSED ") {
                Some("TYPE UNUSED ".len())
            } else if marker_content.starts_with("UNNEEDED assert ") {
                Some("UNNEEDED assert ".len())
            } else if marker_content.starts_with("UNNEEDED admit ") {
                Some("UNNEEDED admit ".len())
...
/// Check if a line in a file has a Veracity result marker above it.
/// Scans upward past any `// Veracity:` lines to collect all markers.
/// Returns the set of marker types found (e.g., "USED", "DEPENDENT", "NEEDED assert").
fn get_markers_above(file: &Path, line_num: usize) -> Vec<String> {
    let content = match std::fs::read_to_string(file) {
        Ok(c) => c,
    ...
    // Scan upward from line_num - 2 (the line before the target, 0-indexed)
    if line_num < 2 { return markers; }
    let mut idx = line_num - 2; // 0-indexed line before target
    loop {
        let trimmed = lines[idx].trim();
        if trimmed.starts_with("// Veracity:") {
            let marker = trimmed["// Veracity:".len()..].trim().to_string();
            markers.push(marker);
            if idx == 0 { break; }
            idx -= 1;
        } else {
...
/// Check if a lemma already has Phase 7 (DEPENDENT/INDEPENDENT) marker
fn has_dependence_marker(file: &Path, start_line: usize) -> bool {
    let markers = get_markers_above(file, start_line);
    markers.iter().any(|m| m == "DEPENDENT" || m == "INDEPENDENT")
}

/// Check if a lemma already has Phase 8 (USED) marker or is UNUSED (commented out)
fn has_necessity_marker(file: &Path, start_line: usize) -> bool {
    let markers = get_markers_above(file, start_line);
    if markers.iter().any(|m| m == "USED") { return true; }
    // Also check if the line itself starts with UNUSED prefix
    let content = match std::fs::read_to_string(file) {
        Ok(c) => c,
        Err(_) => return false,
    };
    ...
    if start_line > 0 && start_line <= lines.len() {
        ...
        return trimmed.starts_with("// Veracity: UNUSED ");
    }
    false
}
...
/// Check if a type already has TYPE USED or TYPE UNUSED marker
fn has_type_marker(file: &Path, start_line: usize) -> bool {
    let markers = get_markers_above(file, start_line);
    if markers.iter().any(|m| m == "TYPE USED") { return true; }
    let content = match std::fs::read_to_string(file) {
        Ok(c) => c,
        Err(_) => return false,
    };
    ...
    if start_line > 0 && start_line <= lines.len() {
        let trimmed = lines[start_line - 1].trim();
        return trimmed.starts_with("// Veracity: TYPE UNUSED ");
    }
    false
}

/// Insert a `// Veracity: <marker>` line before the given line number.
/// The marker line is indented to match the target line.
fn insert_marker_before(file: &Path, line_num: usize, marker: &str) -> Result<()> {
    let content = std::fs::read_to_string(file)?;
    let lines: Vec<&str> = content.lines().collect();
    let mut new_lines: Vec<String> = Vec::new();
...
/// Inserts `// Veracity: DEPENDENT` or `INDEPENDENT` marker before each lemma.
...
fn run_single_file_mode(args: &MinimizeArgs, baseline_time: Duration) -> Result<()> {
    ...
    // Find asserts in the file, sorted by descending line for NEEDED marker safety
    let mut asserts: Vec<_> = find_asserts_in_file(file)?
        .into_iter().filter(|a| !is_protected_assert(a)).collect();
    asserts.sort_by(|a, b| b.line.cmp(&a.line));
    ...
    if args.assert_minimization && !asserts.is_empty() {
        ...
        for (i, assert_info) in asserts.iter().take(test_count).enumerate() {
            ...
            if !args.fresh && has_item_marker(&assert_info.file, assert_info.line, "assert") {
                log!("  [{}/{}] {} at line {} in fn {}... (from prior run, skipping)",
                    i + 1, test_count, assert_info.assert_type, assert_info.line, assert_info.context);
                continue;
            }
    ...
    if args.proof_block_minimization && !proof_blocks.is_empty() {
        ...
        for (i, block) in proof_blocks.iter().take(test_count).enumerate() {
            ...
            if !args.fresh && has_item_marker(&block.file, block.start_line, "proof block") {
                log!("  [{}/{}] lines {}-{} in fn {}... (from prior run, skipping)",
                    i + 1, test_count, block.start_line, block.end_line, block.context);
                continue;
            }
...
fn run_fn_filter_mode(args: &MinimizeArgs, baseline_time: Duration) -> Result<()> {
    ...
    // Sort by descending line number for NEEDED marker safety
    asserts.sort_by(|a, b| a.file.cmp(&b.file).then(b.line.cmp(&a.line)));
    proof_blocks.sort_by(|a, b| a.file.cmp(&b.file).then(b.start_line.cmp(&a.start_line).reverse()));
    ...
    if args.assert_minimization && !asserts.is_empty() {
        ...
        for (i, assert_info) in asserts.iter().take(test_count).enumerate() {
            ...
            if !args.fresh && has_item_marker(&assert_info.file, assert_info.line, "assert") {
                log!("  [{}/{}] {} L{} ({})... (from prior run, skipping)",
                    i + 1, test_count, assert_info.assert_type, assert_info.line, rel.display());
                continue;
            }
    ...
    if args.proof_block_minimization && !proof_blocks.is_empty() {
        ...
        for (i, block) in proof_blocks.iter().take(test_count).enumerate() {
            ...
            if !args.fresh && has_item_marker(&block.file, block.start_line, "proof block") {
                log!("  [{}/{}] lines {}-{} ({})... (from prior run, skipping)",
                    i + 1, test_count, block.start_line, block.end_line, rel.display());
                continue;
            }
...
fn main() -> Result<()> {
    ...
    let (log_path, created_analyses_dir) = init_logging(&args.codebase, args.chapters.first().map(|s| s.as_str()));
    ...
    log!("  mode:               {}", if args.fresh { "fresh (strip all markers)" } else { "resume (preserve markers)" });
    ...
    log!("  // Veracity: NEEDED assert          - Phase 9/10: Assert required for verification");
    log!("  // Veracity: UNNEEDED assert        - Phase 9/10: Assert not needed, left commented");
    log!("  // Veracity: NEEDED admit           - Phase 11: Admit still needed (proof hole)");
    log!("  // Veracity: UNNEEDED admit         - Phase 11: Admit not needed, left commented");
    log!("  // Veracity: NEEDED proof block     - Phase 12: Proof block required");
    ...
    // Phase 0: Handle existing Veracity markers
    let marker_count = count_veracity_markers(&args.codebase)?;
    if marker_count > 0 {
        log!("═══════════════════════════════════════════════════════════════");
        if args.fresh {
            log!("Phase 0: Stripping ALL existing Veracity markers (--fresh)");
        } else {
            log!("Phase 0: Resuming from existing markers");
        }
        log!("═══════════════════════════════════════════════════════════════");
        log!();
        ...
        if args.fresh {
            // --fresh footgun warning
            if !args.danger_mode {
                log!();
                log!("  ╔═══════════════════════════════════════════════════════╗");
                log!("  ║  WARNING: --fresh will strip ALL markers and         ║");
                log!("  ║  re-test from scratch. This discards prior work.     ║");
            ...
            log!("  Checking for incomplete work from prior run...");

            // Step 1: Find files with TESTING-EMPTY-BODY — restore from git
            let files = find_rust_files(&args.codebase);
            let mut empty_body_files = Vec::new();
            for file in &files {
                let content = std::fs::read_to_string(file).unwrap_or_default();
                if content.contains("// Veracity: TESTING-EMPTY-BODY") {
                    empty_body_files.push(file.clone());
                }
            }
            if !empty_body_files.is_empty() {
                log!("  Found {} file(s) with TESTING-EMPTY-BODY (crashed Phase 7):", empty_body_files.len());
                for f in &empty_body_files {
                    let rel = f.strip_prefix(&args.codebase).unwrap_or(f);
                    log!("    git checkout -- {}", rel.display());
                    let _ = std::process::Command::new("git")
                        .current_dir(&args.codebase)
                        .args(["checkout", "--", &rel.to_string_lossy()])
                        .output();
                }
            }

            // Step 2: Find and restore TESTING markers (strip prefix)
            let mut testing_restored = 0;
            for file in &files {
                let content = std::fs::read_to_string(file).unwrap_or_default();
                if !content.contains("// Veracity: TESTING") {
                    continue;
                }
                // Check for partial/contiguous TESTING blocks
                let lines: Vec<&str> = content.lines().collect();
                let mut new_lines: Vec<String> = Vec::new();
                let mut restored_in_file = 0;
                for line in &lines {
                    let trimmed = line.trim();
                    if trimmed.starts_with("// Veracity: TESTING") && !trimmed.contains("TESTING-EMPTY-BODY") {
                        // Extract the marker content to find the original code
                        let after_veracity = &trimmed["// Veracity: ".len()..];
                        // Find the code after the TESTING marker word(s)
                        let code = if after_veracity.starts_with("TESTING assert ") {
                            &after_veracity["TESTING assert ".len()..]
                        } else if after_veracity.starts_with("TESTING admit ") {
                            &after_veracity["TESTING admit ".len()..]
                        } else if after_veracity.starts_with("TESTING proof block ") {
                            &after_veracity["TESTING proof block ".len()..]
                        } else if after_veracity.starts_with("TESTING type ") {
                            &after_veracity["TESTING type ".len()..]
                        } else if after_veracity.starts_with("TESTING ") {
                            &after_veracity["TESTING ".len()..]
                        } else {
                            // Unknown TESTING variant — keep as-is
                            new_lines.push(line.to_string());
                            continue;
                        };
                ...
                if restored_in_file > 0 {
                    std::fs::write(file, new_lines.join("\n") + "\n")?;
                    testing_restored += restored_in_file;
                }
            }
            if testing_restored > 0 {
                log!("  Restored {} TESTING markers to original code", testing_restored);
            }

            // Step 3: Count result markers for progress report
            let mut counts: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
            for file in &files {
                let content = std::fs::read_to_string(file).unwrap_or_default();
                for line in content.lines() {
                    let trimmed = line.trim();
                    if !trimmed.starts_with("// Veracity:") { continue; }
                    let marker = trimmed["// Veracity:".len()..].trim();
                    let key = if marker == "USED" { "USED" }
                        else if marker == "DEPENDENT" { "DEPENDENT" }
                        else if marker == "INDEPENDENT" { "INDEPENDENT" }
                        else if marker == "TYPE USED" { "TYPE USED" }
                        else if marker == "NEEDED assert" { "NEEDED assert" }
                        else if marker == "NEEDED admit" { "NEEDED admit" }
                        else if marker == "NEEDED proof block" { "NEEDED proof block" }
                        else if marker.starts_with("UNUSED ") { "UNUSED" }
                        else if marker.starts_with("UNNEEDED ") { "UNNEEDED" }
                        else if marker.starts_with("TYPE UNUSED ") { "TYPE UNUSED" }
                        else { continue };
                    *counts.entry(key).or_insert(0) += 1;
                }
            }
            let total_result: usize = counts.values().sum();
            if total_result > 0 {
                log!("  Prior results found:");
                for key in &["DEPENDENT", "INDEPENDENT", "USED", "UNUSED",
                            "NEEDED assert", "UNNEEDED", "NEEDED admit",
                            "NEEDED proof block", "TYPE USED", "TYPE UNUSED"] {
                    if let Some(&n) = counts.get(key) {
                        log!("    {}: {}", key, n);
                    }
                }
                log!("  These items will be skipped during testing.");
            } else if testing_restored == 0 && empty_body_files.is_empty() {
                log!("  No incomplete work found. All markers are result markers.");
            }
        }
        log!();
    }
    ...
    if args.apply_lib_broadcasts && !args.dry_run && !lib_recommendations.is_empty() {
        ...
        let (success, stderr, _peak_rss) = run_verus(&args.codebase)?;
        if success {
    ...
    // Track line shifts from marker insertions (shared across phases 7, 8)
    ...
    for (i, ((name, file), variants)) in sorted_groups.iter().enumerate() {
        ...
        // Resume mode: skip if already has DEPENDENT/INDEPENDENT marker
        if !args.fresh && has_dependence_marker(&group_lemmas[0].file, line_shifts.adjust_line(&group_lemmas[0].file, group_lemmas[0].start_line)) {
            let markers = get_markers_above(&group_lemmas[0].file, line_shifts.adjust_line(&group_lemmas[0].file, group_lemmas[0].start_line));
            let prior = markers.iter().find(|m| m.as_str() == "DEPENDENT" || m.as_str() == "INDEPENDENT").unwrap();
            log!("[{}/{}] {}{}... {} (from prior run, skipping)", i + 1, sorted_groups.len(), name, type_info, prior);
            if prior == "DEPENDENT" {
    ...
    for (i, ((name, file), variants)) in sorted_groups.iter().enumerate() {
        ...
        // Resume mode: skip if already has USED/UNUSED marker
        if !args.fresh && has_necessity_marker(&group_lemmas[0].file, line_shifts.adjust_line(&group_lemmas[0].file, group_lemmas[0].start_line)) {
            log!("[{}/{}] {}{}... (from prior run, skipping)", i + 1, sorted_groups.len(), name, type_info);
    ...
    if args.assert_minimization && !args.no_lib_min {
        ...
        // Sort by descending line number within each file so NEEDED marker
        // insertions don't shift line numbers for items not yet tested
        lib_asserts.sort_by(|a, b| a.file.cmp(&b.file).then(b.line.cmp(&a.line)));
        ...
        for (i, assert_info) in lib_asserts.iter().take(test_count).enumerate() {
            // Resume mode: skip if already has NEEDED/UNNEEDED marker
            if !args.fresh && has_item_marker(&assert_info.file, assert_info.line, "assert") {
                log!("[{}/{}] {} L{} in {}... (from prior run, skipping)",
                    i + 1, test_count, assert_info.assert_type, assert_info.line, assert_info.context);
                continue;
            }
    ...
    if args.assert_minimization {
        ...
        // Sort by descending line number within each file
        codebase_asserts.sort_by(|a, b| a.file.cmp(&b.file).then(b.line.cmp(&a.line)));
        ...
        for (i, assert_info) in codebase_asserts.iter().take(test_count).enumerate() {
            ...
            if !args.fresh && has_item_marker(&assert_info.file, assert_info.line, "assert") {
                log!("[{}/{}] {} L{} in {} ({})... (from prior run, skipping)",
                    i + 1, test_count, assert_info.assert_type, assert_info.line, assert_info.context, rel_path.display());
                continue;
            }
    ...
    if args.admit_minimization {
        ...
        // Sort by descending line number within each file
        all_admits.sort_by(|a, b| a.file.cmp(&b.file).then(b.line.cmp(&a.line)));
        ...
        for (i, admit_info) in all_admits.iter().take(test_count).enumerate() {
            ...
            // Resume mode: skip if already has NEEDED/UNNEEDED marker
            if !args.fresh && has_item_marker(&admit_info.file, admit_info.line, "admit") {
                log!("[{}/{}] admit L{} in {} ({})... (from prior run, skipping)",
                    i + 1, test_count, admit_info.line, admit_info.context, rel_path.display());
                continue;
            }
    ...
    if args.proof_block_minimization {
        ...
        // Sort by descending line number within each file
        all_proof_blocks.sort_by(|a, b| a.file.cmp(&b.file).then(b.start_line.cmp(&a.start_line).reverse()));
    ...
    if args.type_minimization {
        ...
        if all_types.is_empty() {
            ...
            for (i, type_info) in all_types.iter().take(test_count).enumerate() {
                ...
                // Resume mode: skip if already has TYPE USED/UNUSED marker
                if !args.fresh && has_type_marker(&type_info.file, type_info.start_line) {
                    log!("  [{}/{}] {}{} {}... (from prior run, skipping)",
                        i + 1, test_count, pub_str, type_info.kind, type_info.name);
                    continue;
                }
