...
//! veracity-paths-transform - Replace content at a path with new text
//!
//! Usage:
//!   veracity-paths-transform -v <file.vp> -s <source.rs> -p <path> -t <text> [-o <output.rs>]
//!   veracity-paths-transform -v <file.vp> -s <source.rs> -p <path> -f <file> [-o <output.rs>]
//!
//! Path is a substring match. Replaces the span at that path with the new text.
...
fn main() -> Result<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                eprintln!("Usage: veracity-paths-transform -v <file.vp> -s <source.rs> -p <path> (-t <text> | -f <file>) [-o <output.rs>]");
                eprintln!("  Replaces the span at the matching path with new text.");
                std::process::exit(0);
            }
            _ => {
                i += 1;
    ...
    let new_text = if let Some(t) = text {
        t
    ...
    let output = format!("{}{}{}", &src_content[..byte_start], new_text, &src_content[byte_end..]);
    ...
    fs::write(out, &output)?;
    eprintln!("veracity-paths-transform: replaced {}:{}–{}:{}", line_s, col_s, line_e, col_e);
